...
bool PointInMap(const Vector2& point)
{
	...
	for (const auto& objectLayer : CurrentMap.ObjectLayers)
	{
...
bool CheckCollisionLineRec(const Vector2& startPoint, const Vector2& endPoint, const Rectangle& rectangle)
{
	...
	// top
	if (CheckCollisionLines(startPoint, endPoint, Vector2{ rectangle.x, rectangle.y }, Vector2{ rectangle.x + rectangle.width, rectangle.y }, &collisionPoint))
		return true;

	// right
	if (CheckCollisionLines(startPoint, endPoint, Vector2{ rectangle.x + rectangle.width, rectangle.y }, Vector2{ rectangle.x + rectangle.width, rectangle.y + rectangle.height }, &collisionPoint))
		return true;

	// bottom
	if (CheckCollisionLines(startPoint, endPoint, Vector2{ rectangle.x, rectangle.y + rectangle.height }, Vector2{ rectangle.x + rectangle.width, rectangle.y + rectangle.height }, &collisionPoint))
		return true;

	// left
	if (CheckCollisionLines(startPoint, endPoint, Vector2{ rectangle.x, rectangle.y }, Vector2{ rectangle.x, rectangle.y + rectangle.height }, &collisionPoint))
		return true;
...
void LoadMap(const char* file)
{
	...
	{
		TileLayer* lastTileLayer = CurrentMap.TileLayers.back();

		MapBounds.width = (lastTileLayer->Size.x * lastTileLayer->TileSize.x);
		MapBounds.height = (lastTileLayer->Size.y * lastTileLayer->TileSize.y);
